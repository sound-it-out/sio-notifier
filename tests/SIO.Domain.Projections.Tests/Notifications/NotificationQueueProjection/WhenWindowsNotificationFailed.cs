...
{
    public class WhenWindowsNotificationFailed : ProjectionSpecification<Projections.Notifications.NotificationQueueProjection>
    {
        ...
        {
            yield return new WindowsNotificationQueued(_aggregateId, 1, _template, _payload, _tags);
            yield return new NotificationFailed(_aggregateId, 2, _error);
        }
        ...
        {
            var notification = Context.Find<NotificationQueue>(_aggregateId);
            notification.Type.Should().Be(NotificationType.Windows);
        }
        ...
        {
            var notification = Context.Find<NotificationQueue>(_aggregateId);
            notification.Version.Should().Be(2);
        }
    }
}
