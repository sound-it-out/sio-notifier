...
namespace SIO.Domain.Tests.Translation.EventHandlers.TranslationFailedEventHandler.HandleAsync
{
    public class WhenExecuted : EventHandlerSpecification<TranslationFailed>
    {
        private readonly TranslationFailed _event = new TranslationFailed(Guid.NewGuid().ToSequentialGuid(), Guid.NewGuid().ToSequentialGuid(), Guid.NewGuid().ToSequentialGuid(), 2, "Test error");
        protected override TranslationFailed Given()
        {
            return _event;
        }
        ...
        public void QueueAndroidNotificationCommandShouldBeDispatchedWithCorrectTemplate()
        {
            var commandDispatcher = (FakeCommandDispatcher)_serviceProvider.GetRequiredService<ICommandDispatcher>();
            var command = (QueueAndroidNotificationCommand)commandDispatcher.Commands.First(c => c.GetType() == typeof(QueueAndroidNotificationCommand));
            command.Template.Should().Be(nameof(TranslationFailed));
        }
